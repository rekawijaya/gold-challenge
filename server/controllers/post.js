...
let self = module.exports ={
    ...
    updatePosting: async function (req, res) {
        ...
        if (getUser.length > 0 && getIdPosting.length > 0) {
            const dataUpdate = {
                description: description,
                file: file.path, 
                date: date,
            ...
            await query.update('posting', dataUpdate, {id_user: id_user, id: id_posting})
    ...
    dletePosting: async function (req, res){
        ...
        const getUser     = await query.select('posting', {id_user: id_user})

        if (getPosting.length > 0 && getUser.length > 0) {
            await query.delete('posting', {id: id_posting, id_user: id_user})
